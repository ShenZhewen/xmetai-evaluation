"""
Fengshun v2.0 MJO 三变量完整处理流水线。

该脚本合并以下两个原脚本的功能，但不依赖也不修改原脚本：
  1. fengshun_v2.0_cal_anom_ttr_u200_u850_2024（same12）.py
  2. mjo_data_combine（略有不同）.py
...
RAW_DATA_BASE_DIR = Path(                                               ##变量格式：ttr(member=11, lead_time=60, lat=121, lon=240)，空间分辨率1.5°
    "/gpu/zhouchg/wangchp/fengshun_v2.0_u200_u850_ttr_2004-2024"              ##  u200(member=11, lead_time=60, lat=121, lon=240)，空间分辨率1.5°
...
INTERMEDIATE_OUTPUT_DIR = Path("/gpu/zhouchg/liuzh/MJO/fengshun_v2.0")      ##变量格式（member已平均，还未插值）：ttr(lead_time=60, lat=121, lon=240)，空间分辨率1.5°
...
FINAL_OUTPUT_FILE = Path("/gpu/zhouchg/liuzh/MJO/fengshun_v2.0_output/fengshun2.0_anom_daily_var3_2p5_2024.nc")    
...
def save_intermediate_dataset(
    ...
    dataset.attrs.update({
        "source": "Fengshun v2.0 Ensemble Forecast System",
...
def interpolate_and_merge(
    ...
    final_dataset.attrs.update({
        "description": (
            "Fengshun v2.0 merged anomaly data; TTR converted to OLR and "
...
def main() -> None:
    ...
    log.info("=" * 70)
    log.info("Fengshun v2.0 MJO完整流水线: CLIM → ANOM → MERGE 2.5°")
