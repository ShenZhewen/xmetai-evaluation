...
cfg = EvalConfig(
    ...
    #
    # 分纬度带评估：regions 里每个带在 (起报, 时效) 样本之外再各出一行，
    # 长表里用 region 列区分（全球行该列为空）。边界是闭区间。标量指标
    # （rmse/acc/activity）逐带出分；谱类（zonal_spectrum）不分区、只出
    # 全球行——掩掉的子区域上算出来的不是同一个物理量。不分区就整个
    # 键删掉（或设 None），输出回到只有全球行。下面用的是经典三分带：
    # 热带 / 两半球中高纬，改边界直接改数字。
    options={
        "sample_by": "init_lead",
        "regions": {
