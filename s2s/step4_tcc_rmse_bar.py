...
def process_variable(variable: str, year: int) -> None:
    ...
    init_dates = generate_init_dates(
        year,
        int(INIT_START_MMDD[:2]), int(INIT_START_MMDD[2:]),
        int(INIT_END_MMDD[:2]), int(INIT_END_MMDD[2:]),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    # 起报日期区间（原代码硬编码 9/23-12/31，集成后改由 config 控制，默认全年）
    parser.add_argument("--init-start", type=str, default="0101", help="起报开始日期 MMDD（默认全年 0101）")
    parser.add_argument("--init-end", type=str, default="1231", help="起报结束日期 MMDD（默认全年 1231）")
...
def main() -> None:
    ...
    global INIT_START_MMDD, INIT_END_MMDD
    INIT_START_MMDD = args.init_start
    INIT_END_MMDD = args.init_end
