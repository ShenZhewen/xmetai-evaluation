...
# ── 13. 离线地图数据目录（含 shapefiles/…；None=默认用脚本同级 cartopy/，再没有就让 cartopy 自己找）──
CARTOPY_DIR = None
...
def plot_global_maps(
    ...
    try:
        ...
        # 离线 shapefile 目录改为可配置（原代码硬编码 /home/nmic/project/cartopy）
        if CARTOPY_DIR:
            cartopy.config["pre_existing_data_dir"] = Path(CARTOPY_DIR)
            cartopy.config["data_dir"] = Path(CARTOPY_DIR)
        ##########################################################################################################################
...
示例:
  ...
  python CRA_v1.5_process.py --vars t2m --year-start 2021 --year-end 2024
    ...
    p.add_argument(
        "--cartopy-dir", type=str, default=None,
        help="离线 Natural Earth shapefile 目录（含 shapefiles/…）；默认用脚本同级 cartopy/，不存在则用 cartopy 默认",
    )
    return p.parse_args()
...
def main():
    ...
    global CARTOPY_DIR
    # cartopy 离线数据目录：CLI 优先，其次脚本同级 cartopy/，都没有则留空走 cartopy 默认
    if args.cartopy_dir:
        CARTOPY_DIR = args.cartopy_dir
    else:
        _local = Path(__file__).resolve().parent / "cartopy"
        CARTOPY_DIR = str(_local) if _local.is_dir() else None
