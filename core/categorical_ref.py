...
BSS 参考：给 ``--ref_result`` 用外部逐 6h 气候概率，缺省回退样本气候频率。
...
用法：
    ...
    # BSS 用外部逐 6h 气候概率（ref_result/MMDDHH.000，北京时，站号+4阈值概率）
    python run_categorical.py --root ts_sample/fuxi_ens_output \
        ...
        --ref_result ts_sample/r0/ref_result
...
def run_categorical(root, station_dir, window_ts=(6.0, 24.0),
    ...
    ref_result: 逐 6h 气候概率参考目录（ref_result/MMDDHH.000，北京时，仅用于 6h AROC 窗）；
        缺省回退样本气候频率 BSS。
    """
    t0 = time.time()
    print(f"[categorical] Starting evaluation", flush=True)
    print(f"[categorical]   root: {root}", flush=True)
    print(f"[categorical]   station_dir: {station_dir}", flush=True)
    print(f"[categorical]   window_ts: {window_ts}", flush=True)
    print(f"[categorical]   window_aroc: {window_aroc}", flush=True)
    print(f"[categorical]   workers: {workers}", flush=True)

    thresholds = thresholds or DEFAULT_THRESHOLDS
    whitelist = load_station_whitelist(station_list) if station_list else None

    print(f"[categorical] Loading dataset...", flush=True)
    ds = ClassDataset(root, var=var, step=step, init_hour=init_hour,
                      workers=workers)

    if start_date or end_date:
        lo = dt.datetime.strptime(start_date, "%Y%m%d") if start_date else None
        hi = dt.datetime.strptime(end_date, "%Y%m%d") if end_date else None
        ds._init_dates = [d for d in ds.init_dates if (lo is None or d >= lo) and (hi is None or d <= hi)]
        print(f"[categorical] Filtered to {len(ds._init_dates)} dates ({start_date} to {end_date})", flush=True)

    if ds.lead_hours.size == 0:
        raise ValueError("数据集无时效文件")

    is_ensemble = ds.n_members > 1
    print(f"[categorical] Mode: {'ensemble (%d members)' % ds.n_members if is_ensemble else 'deterministic'}", flush=True)
    print(f"[categorical] Found {len(ds.init_dates)} init dates, {len(ds.lead_hours)} lead hours", flush=True)

    paths = scan_station_dir(station_dir)
    print(f"[categorical] Scanned {len(paths)} station files", flush=True)
    station_cache = {}
    ...
    ref_sums = {}        # (W, lead, ti) -> [Σ(p_clim−o)², n]，仅当 ref_result
    bs_sums = {}         # (W, lead, ti) -> [Σ(p_fc−o)², n]，仅当 ref_result（与 BS_ref 同批样本）
    ...
    print(f"[categorical] Processing {len(ds.init_dates)} init dates...", flush=True)
    n_dates = len(ds.init_dates)
    t_loop = time.time()
    for idx, init_date in enumerate(ds.init_dates, start=1):
        if idx == 1 or idx % 5 == 0 or idx == len(ds.init_dates):
            elapsed = time.time() - t_loop
            eta = elapsed / idx * (n_dates - idx)
            print(f"[categorical] Processing date {idx}/{n_dates}: "
                  f"{init_date.strftime('%Y%m%d')} "
                  f"({100*idx/n_dates:.1f}%, ETA {eta/60:.1f}m)", flush=True)
        ...
        for W, lead, i_start, i_end in all_plans:
            ...
            if members is not None and (W, lead) in aroc_keys:
                ...
                # BSS 外部参考：按验证时刻北京时查逐站概率，缺 ref_result 站气候概率按 0 兜底（BS 与 BS_ref 同批样本）
                if ref_prob is not None and W == 6.0:
                    probs = ref_prob.probabilities(v_bjt, acc["id"])   # (n, 4)
                    keep = (np.isfinite(acc["rain"])
                    ...
                    for ti, (_nm, thr) in enumerate(aroc_thr[W]):
                        ...
                        pc = np.nan_to_num(probs[:, ti], nan=0.0)      # 缺 ref_result → pRef=0
    ...
    # 输出 TS（确定性 / 集合均写）
    print(f"[categorical] Finalizing TS results...", flush=True)
    ts_rows = []
    for W, lead, _, _ in ts_plans:
    ...
    print(f"[categorical] Wrote TS results: {ts_csv}", flush=True)

    # 输出超越式 AROC / BS / BSS（仅集合；逐 (窗口, 时效, 阈值) + AVG 综合行）
    ab_df = None
    ab_csv = None
    if peh_tables is not None:
        print(f"[categorical] Finalizing AROC/BS/BSS results...", flush=True)
        ...
        for (W, lead) in sorted(per):
            ...
            for ti, (nm, thr) in enumerate(thr_list):
                ...
                if ref_prob is not None and W == 6.0:
                    ...
                    bs_s, bs_c = bs_sums.get((W, lead, ti), (0.0, 0))
                    bs = bs_s / bs_c if bs_c else np.nan   # BS 改用 ref_result-covered 站，与 BS_ref 同批
                    bss = (1 - bs / bs_ref
        ...
        print(f"[categorical] Wrote AROC/BS/BSS results: {ab_csv}", flush=True)

    elapsed = time.time() - t0
    print(f"[categorical] Completed in {elapsed:.1f}s ({elapsed/60:.2f}m)", flush=True)
    meta_windows = {}
    ...
    meta = {
        ...
        "ref_result": ref,
        "bss_reference": ("外部逐 6h 气候概率（--ref_result，逐站×阈值，缺站按 0 兜底）"
...
def _self_test() -> int:
    ...
    try:
        ...
        # 外部 ref_result（逐 6h 气候概率，BS_ref=mean((p_clim−o)²)）
        refdir = os.path.join(tmp, "ref_result")
        os.makedirs(refdir)
        ...
        res3, _ = run_categorical(os.path.join(tmp, "ens"), sdir,
                                     ...
                                     verbose=False)
        bb = res3["aroc_bss"]
        check("集合：外部 ref_result BS_ref=mean((p_clim−o)²)（p_clim=0.5 → 0.25）",
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--ref_result", default='/workspace/data/worm/r0/ref_result',
                    help="BSS 外部逐 6h 气候概率目录（ref_result/MMDDHH.000，北京时，"
