...
| 集合 | `weather_rmse_<模型>_ens` | `--summarize-ens` 的批次归档 | `weather_rmse_ens.md` | ⛔ **还没做** |
| 谱检验补充 | `weather_rmse_wave_<模型>` | 批次归档 + `spherical_bands_*` | `weather_rmse_wave.md` | ⛔ **还没做** |

**只有 `_single` 这一条能跑。** 后两条的 `visualization/` 里既没有
`ens_report.py` 也没有 wave 的渲染器，`scripts/` 下也只有 `generate_det_report.py`。
骨架是**格式契约**，不是能跑的东西——调用时要明确报错，不要静默套用 `_single` 的模板。
...
6. **`_wave` 的球谐带没有数据来源**：`vfc/metrics/spectrum.py` 只有纬向 FFT，
   球谐停在「升级备忘」（`regr_pair.py:145` 也这么写）。骨架里 5.1–5.3 三节
   目前**渲染不出来**，要写就得先补球谐的实现。
...
6. 不要在 `_ens` / `_wave` 的渲染器还没做时，拿 `_single` 的模板顶上——
   **拿错模板比报错危险**；
