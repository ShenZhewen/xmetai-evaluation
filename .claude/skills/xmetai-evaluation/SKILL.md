...
description: >
  ...
  覆盖两条链路：降水分类检验（TS 系列，含集合）与确定性连续场 / RMSE 批次家族。
...
| `summary.csv` + `batch_meta.json` + `<YYYYMMDD>/` | **RMSE 批次家族** | `scripts/generate_det_report.py`，见 `rmse-batch-evaluation.md` |
| `_meta.json` 里有 `babj` / `tcid` / `forecast_type` | **台风路径** | 模板还没做，明确报错 |
...
> **后两份的渲染器都还没做**。归档形状、派生指标（相对 RMSE / FA 偏差 / 频谱对数 RMS）
...
> `assets/templates/` 现在只剩上面五份。**确定性连续场（`weather_field_scores`）
...
- **已知缺口**（写在这里免得下次重新发现）：
  ...
  2. `weather_rmse_ens` / `weather_rmse_wave` 的渲染器还没做；
  3. 概率评分（AROC / BSS）取数与绘图那一层还没做，`weather_ts_ens_prob` 标着 `[BLOCKED]`；
  4. 球谐带功率在代码里不存在，只有「升级备忘」。
- `agents/` 目前是空的。
