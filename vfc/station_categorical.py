# -*- coding: utf-8 -*-
"""站点层：diamond3 逐小时观测 + BSS 气候概率参考（ref_result/MMDDHH.000）。
...
class RefProb(object):
    ...
    def __init__(self, ref_dir):
        ...
        if not self._index:
            raise ValueError("ref_result 目录 %s 下无 .000 文件" % self.ref_dir)
    ...
    def probabilities(self, when, station_ids):
        ...
        缺 ref_result 的站对应行置 NaN（4 列全 NaN），由调用方决定跳过。
        """
        key = when.strftime("%m%d%H")
        if key not in self._index:
            raise ValueError("ref_result 缺少时次 %s（%s）" % (key, when))
