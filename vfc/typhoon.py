...
def read_babj_analyses(path):
    ...
    try:
        ...
        raise TyphoonDataError("解析 %s 失败: %s" % (path, e))
    analyses = {}
    for r in rows:
        if int(round(r[4])) != 0:                     # 只要实况行
            continue
        t = _dt.datetime(int(r[0]), int(r[1]), int(r[2]), int(r[3]))
        analyses[t] = {"lon": float(r[5]), "lat": float(r[6]),
                       "pmin_hpa": float(r[7]), "vmax_ms": float(r[8])}
    if not analyses:
...
def great_circle_km(lat1, lon1, lat2, lon2):
    ...
    lat1, lon1, lat2, lon2 = (np.deg2rad(np.asarray(x, dtype="f8"))
...
def find_center_by_slp(msl, glat, glon, clat, clon, half_lat, half_lon):
    ...
    if not m.any():
        raise TyphoonDataError("搜索框 (%g,%g)±(%g,%g) 越出网格"
                               % (clat, clon, half_lat, half_lon))
...
def diagnose_track(msl, glat, glon, init_lat, init_lon, leads_hours,
                   wind=None, center_half=3.0, early_half=4.0,
                   early_hours=12.0, intensity_half=5.0):
    """从实况初始位置起链式诊断台风中心与强度。
    ...
    for k in range(T):
        ...
        lat_c, lon_c, pmin = find_center_by_slp(msl[k], glat, glon,
                                                plat, plon, half, half)
        if wind is not None:
            vmax = max_wind_in_box(wind[k], glat, glon, lat_c, lon_c,
...
def along_cross_track(lat0, lon0, lat_b, lon_b, lat_p, lon_p):
    ...
    def _alpha(la0, lo0, la, lo):
        ...
        return math.atan2(dlon * math.cos((la + la0) / 2 / 180 * math.pi),
                          dlat)
    dalpha = _alpha(lat0, lon0, lat_p, lon_p) - _alpha(lat0, lon0,
                                                       lat_b, lon_b)
...
def match_errors(track, leads_hours, init_utc, babj, tz_shift=8.0):
    ...
    for k, tau in enumerate(leads_hours):
        ...
        if ob is None:
            rows.append({"lead_h": float(tau), "valid_bjt": str(v_bjt),
                         "fcst_lat": f_lat, "fcst_lon": f_lon,
                         "fcst_pmin_hpa": f_p, "fcst_vmax_ms": f_v,
                         "obs_lat": np.nan, "obs_lon": np.nan,
                         "obs_pmin_hpa": np.nan, "obs_vmax_ms": np.nan,
                         "track_err_km": np.nan, "at_km": np.nan,
                         "ct_km": np.nan, "wind_err_ms": np.nan,
                         "pmin_err_hpa": np.nan})
            continue
        ...
        if prev_babj is None:                          # 首个配对时次
            ...
            at, ct = along_cross_track(prev_babj[0], prev_babj[1],
                                       o_lat, o_lon, f_lat, f_lon)
        rows.append({"lead_h": float(tau), "valid_bjt": str(v_bjt),
                     "fcst_lat": f_lat, "fcst_lon": f_lon,
                     "fcst_pmin_hpa": f_p, "fcst_vmax_ms": f_v,
                     "obs_lat": o_lat, "obs_lon": o_lon,
                     "obs_pmin_hpa": ob["pmin_hpa"],
                     "obs_vmax_ms": ob["vmax_ms"],
                     "track_err_km": err, "at_km": at, "ct_km": ct,
                     "wind_err_ms": f_v - ob["vmax_ms"]
                     if np.isfinite(f_v) else float("nan"),
